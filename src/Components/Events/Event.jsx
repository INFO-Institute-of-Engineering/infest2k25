...
const Event = () => {
  ...
  const renderLink = (category, label) => (
    <Link
      to={`/event/${category}`}
      className={`cursor-pointer ${showEvent === category && "bg-gradient-to-r from-green-500 to-blue-500 text-white"} duration-700 ease-linear transition px-[2%] py-[1%] rounded-[25px] text-[15px]`}
